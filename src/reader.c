...
void print_number (size_t x)
{
  ...
  fprintf (stderr,"%d", y[0]);
  for (k = 1; k < triplets; ++k)
   {
    fprintf (stderr,",%03d", y[k]);
   }
}
...
void rewind_files (const char * file1, const char * file2)
{
  ...
  if (forward_file_type == PEAR_FILETYPE_GZIP)
  {
    ...
    if (!gz_fd1 || !gz_fd2)
    {
      fprintf (stderr,"Error: creating gzip file handlers\n");
      exit(1);
    }
  }
  #endif
                                      ...
                                      memBlock * sec_rev)
{
  ...
  #ifdef __DEBUG__
  fprintf (stderr,"Size of reads structure: ");
  ...
  #ifdef __DEBUG__
  fprintf (stderr,"Size of used memory: ");
  ...
  #ifdef __DEBUG__
  fprintf (stderr,"Size of unused memory: ");
